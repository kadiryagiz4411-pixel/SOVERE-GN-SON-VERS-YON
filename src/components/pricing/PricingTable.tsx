...
import { PLANS, SINGLE_PASS } from "@/config/pricing";
...
interface PricingTableProps {
  ...
  isLoggedIn?: boolean;
  /** If true, shows all 4 plans. Default: true */
...
export function PricingTable({
  ...
  const plans = showEnterprise ? PLANS : PLANS.filter(p => p.id !== "enterprise");
    ...
    <div className={cn("w-full", className)}>
      ...
      <div className="flex items-center justify-center gap-4 mb-12">
        ...
        <button
          ...
          className={cn(
            "relative w-14 h-7 rounded-full transition-colors duration-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-violet-500",
    ...
    <div className="mt-8 mx-auto max-w-3xl">
      <div className="relative flex flex-col sm:flex-row items-center gap-5 px-6 py-5 rounded-2xl border border-dashed border-slate-600 bg-slate-900/60 hover:border-slate-500 transition-colors">
        ...
        <div className="flex-shrink-0 flex flex-col items-center justify-center w-20 h-20 rounded-2xl bg-teal-500/10 border border-teal-500/30 mt-2 sm:mt-0">
          <span className="text-2xl font-black text-teal-400 leading-none">$4.99</span>
        ...
        <div className="flex-1 text-center sm:text-left">
          <p className="text-base font-bold text-slate-100">
            ...
            <span className="text-teal-400">Single Pass for $4.99</span>
          </p>
          <ul className="mt-2 flex flex-wrap justify-center sm:justify-start gap-x-4 gap-y-1">
            {SINGLE_PASS.features.map((f, i) => (
